...
**Phase 1 — OpenShift** — build per-vault stores, seed openshift token, repoint every ES + PushSecret, resolve the pac-tenant span blocker (§6 risk 2). Add cluster-side **kubeconfig PushSecrets** for `ocp`/`hub` → `claude` (replaces the AAP publish for those; gap 1). One PR; verify stores Valid + apps Synced.
...
**Phase 3 — AAP/Ansible** — re-mint AAP Connect credential 16 with the aap scope; flip `vault='awx'`→`vault='lab_*'` in `auth.yml`, `credentials.yml`, `workflows.yml`, `notifications.yml`, `hermes.yml`, host_vars, and the routeros/rhsm/terraform/letsencrypt playbooks (tailscale deferred). **Keep `request_timeout: "30"`.** **Retire the two AAP 1P-write paths** (gaps 1 & 2): remove `publish-kubeconfig-1password.yaml`'s claude write (kubeconfigs now cluster-pushed) and `sync_1pasword_secrets.yml`'s SA-token write (OpenShift PushSecrets own it). Verify with `onepassword_smoke_test` JT, then `aap_configure_all`.
...
|---|---|---|---|---|---|---|---|
| 1 | **aap** | write `op://claude/<cluster>-kubeconfig` (`publish-kubeconfig-1password.yaml`, `install-k3s-cluster.yml`) | claude | **Write** | Read | **RETIRE** | **Resolved via the PushSecret model (decision):** retire the Ansible kubeconfig-publish write. Each cluster publishes its own kubeconfig as a PushSecret — `ocp`/`hub` (openshift entity, claude RW) → `claude`; `rk8s` (rk8s entity, can't write `claude`) → **`lab_rk8s`** (readers repoint — see §5/consequence). AAP then never writes `claude`. |
| 2 | **aap** | write SA tokens (`sync_1pasword_secrets.yml` → today writes to `awx`) | lab_serviceaccounts | **Write** | Read | **RETIRE** | **Confirmed retire.** Delete the 1P-write in `sync_1pasword_secrets.yml`; OpenShift PushSecrets own SA-token writes to `lab_serviceaccounts`. aap keeps Read-only there (which it has). |
| 3 | **rk8s** | any future PushSecret (SA token / generated secret) | lab_serviceaccounts | Write | Read | **IGNORED (for now)** | Deferred per decision — rk8s writes only `lab_rk8s` today (its kubeconfig push, gap 1). Revisit if rk8s ever needs to publish to a shared vault. |
...
**Status:** #1 and #2 both **retired via the PushSecret model** (clusters publish their own kubeconfigs/SA-tokens; AAP stops writing `claude`/`awx`). #3 **ignored for now**. #4 (tailscale copy) handled when rk8s is un-deferred. Remaining are over-grants (#6/#7) to tighten opportunistically and the transitional `awx` read (#10) removed in Phase 5.

**Consequence of retiring #1:** `rk8s-kubeconfig` moves `claude` → **`lab_rk8s`** (rk8s can't write `claude`). Readers repoint: `igou-devenv/envs/rk8s.env`, the `use rk8s` helper, and any `op://claude/rk8s-kubeconfig` reference. The `ocp`/`hub` kubeconfigs stay in `claude` (openshift-pushed). Confirm this vault move for `rk8s-kubeconfig` before executing, or keep it in `claude` by having an openshift-side job publish it.
