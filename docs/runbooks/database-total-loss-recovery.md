...
(PRs #387/#388). Backup inventory re-verified against the live cluster on
**2026-07-31** (adds `keycloak-pg`).
...
| `keycloak-pg` (db `keycloak`) | `keycloak` | CNPG Cluster | daily base + WAL → `s3://cnpg-backups/keycloak-pg` | §3 — RHBK/SSO; created after the 2026-07-03 DR, so its READ serverName differs (see §3) |
...
4. **Wait for each recovery to complete** (§4), then let / make the consuming
   apps (forgejo, quay, rhdh, rhbk/keycloak) roll. Roll Keycloak before RHDH —
   RHDH logs in through it.
...
- **READ side** (`externalClusters[].plugin.parameters.serverName`): the
  ...
  manifest. **As of 2026-07-31 that is `<cluster>-r20260704`** for `forgejo-pg`,
  `quay-pg`, and `rhdh-pg` (the post-2026-07-03-recovery timelines). For those
  three it is *not* the cluster name — the bare `<cluster>` prefixes hold the
  pre-2026-07-03 archive, frozen at 2026-07-02, and restoring from them silently
  resurrects month-old data. **`keycloak-pg` is the exception**: it was created
  after the DR and has never been recovered, so it carries no explicit
  `serverName` and its READ side is the bare `keycloak-pg`. Do not apply the
  `-r20260704` rule to it.
