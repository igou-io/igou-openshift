...
```text
Deployment/sonarr (replicas: 1)
├── /config -> PVC/sonarr-config -> PV/sonarr-config-iscsi
└── /data   -> PVC/sonarr-data -> NFS /mnt/cold/media/data
```

The Service is available internally at `sonarr.sonarr.svc:8989`, and a
trusted-LAN HTTPRoute exposes `sonarr.lan.igou.systems`. The shared `/data`
filesystem is the same TrueNAS export mounted by qBittorrent and Radarr. This
...
The pre-migration source application reported version `4.0.18.2978`, SQLite,
